...
export default async function SearchPage({
  ...
  searchParams: Promise<{ q?: string; type?: string; dept?: string; sort?: string; page?: string }>
}) {
  const { q = '', type = '', dept = '', sort = 'relevance', page: pageParam = '1' } = await searchParams
  const page = Math.max(1, parseInt(pageParam, 10) || 1)

  const index = await getSearchIndex()
  let results = runSearch(index, q)
  ...
  const productResults = pagedResults.filter((r) => r.type === 'product')
  const postResults = pagedResults.filter((r) => r.type === 'post')
  const deptResults = pagedResults.filter((r) => r.type === 'department')
  ...
  function buildPageUrl(p: number) {
    const params = new URLSearchParams({ q, type, dept, sort })
            ...
            <div className="flex flex-col gap-8 lg:flex-row lg:gap-12">
                    ...
                    <li>
                      <Link
                        ...
                        className={cn(
                          ...
                          !dept && 'bg-primary/10 font-semibold text-primary',
                        )}
                      >
                        All departments
                      </Link>
                    ...
                    {DEPARTMENTS.map((d) => (
                      <li key={d.id}>
                        <Link
                          ...
                          className={cn(
                            'block rounded-xl px-3 py-2 text-sm font-medium transition-colors hover:bg-muted',
                            dept === d.name && 'bg-primary/10 font-semibold text-primary',
                          )}
                        >
                          {d.name}
                        </Link>
              ...
              <div className="min-w-0 flex-1">
                      ...
                      {productResults.map((result) => (
                        ...
                        >
                          ...
                          <div className="flex flex-1 flex-col p-4">
                            <span
                              className={cn(
                                'mb-1.5 inline-block self-start rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide',
                                result.tagColor,
                              )}
                            >
                              {result.tag}
                ...
                {postResults.length > 0 && (
                  <section className="mb-10">
                    {!type && (
                      <h2 className="mb-4 text-lg font-bold text-foreground">Articles</h2>
                    )}
                ...
                {deptResults.length > 0 && (
                  <section>
                    {!type && (
                      <h2 className="mb-4 text-lg font-bold text-foreground">Departments</h2>
                    )}
                    <div className="grid gap-4 sm:grid-cols-2">
