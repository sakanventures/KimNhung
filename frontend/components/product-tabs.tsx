...
type NutritionRow = { label: string; value: string; indent?: boolean }
type Nutrition = { servingSize: string; calories: number; rows: NutritionRow[] }
type Origin = { country: string; imported: boolean; brandNativeScript?: string; note: string }
...
export function ProductTabs({ description, highlights, nutrition, origin }: Props) {
  const tabs: { id: Tab; label: string }[] = [
    { id: 'details', label: 'Details' },
    ...(nutrition ? [{ id: 'nutrition' as Tab, label: 'Ingredients & Nutrition' }] : []),
    ...(origin ? [{ id: 'origin' as Tab, label: 'Origin & Sourcing' }] : []),
  ]
          ...
          <div className="max-w-sm">
            ...
            <div className="overflow-hidden rounded-2xl border border-border">
              <div className="flex items-baseline justify-between border-b border-border bg-muted/50 px-4 py-3">
                ...
                <span className="text-2xl font-black text-foreground">{nutrition.calories}</span>
              </div>
            ...
            <div className="flex items-start gap-4 rounded-2xl border border-border bg-card p-5">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-base font-bold text-foreground">{origin.country}</span>
                  <span
